...
import { Activity, ArrowRight, Lock, ArrowLeft, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0A1628] flex items-center justify-center p-4">
      <div className="bg-white rounded-[2rem] p-10 w-full max-w-md animate-scale-in shadow-2xl">
        <div className="w-14 h-14 bg-[#059669]/10 rounded-2xl flex items-center justify-center mb-6">
          <Lock size={28} className="text-[#059669]" />
        </div>
        <h2 className="text-2xl font-extrabold text-[#0A1628] tracking-tight" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Reset password</h2>
        <p className="text-[#94A3B8] font-medium mt-1 text-sm mb-8">Enter the OTP sent to <strong className="text-[#1E293B]">{email || 'your email'}</strong></p>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="text-xs font-bold text-[#64748B] uppercase tracking-wider mb-2 block">OTP Code</label>
          ...
          <div>
            <label className="text-xs font-bold text-[#64748B] uppercase tracking-wider mb-2 block">New password</label>
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2 text-sm bg-[#059669] hover:bg-[#047857]">
