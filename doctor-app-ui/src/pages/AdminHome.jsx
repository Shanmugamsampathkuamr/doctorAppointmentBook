...
import { Users, Shield, RefreshCcw, Heart, UserX, UserCheck, Clock, AlertTriangle, Plus, X, Stethoscope } from 'lucide-react';
    ...
    <div className="animate-fade-in space-y-8 max-w-7xl">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-[#7A2E1A] to-[#4F1B0D] p-6 rounded-2xl text-white">
          <Users size={28} className="text-[#C9953C] mb-3" />
          <p className="text-3xl font-extrabold" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{users.length}</p>
          <p className="text-sm text-[#C9953C]/70 font-semibold mt-1">Total Users</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-[#E8DDD3] card-hover">
          <UserCheck size={28} className="text-[#2D6A4F] mb-3" />
          <p className="text-3xl font-extrabold text-[#2B1E16]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{roleCounts['PATIENT'] || 0}</p>
          <p className="text-sm text-[#9E8E82] font-semibold mt-1">Patients</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-[#E8DDD3] card-hover">
          <Heart size={28} className="text-[#C9953C] mb-3" />
          <p className="text-3xl font-extrabold text-[#2B1E16]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{roleCounts['DOCTOR'] || 0}</p>
          <p className="text-sm text-[#9E8E82] font-semibold mt-1">Doctors</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-[#E8DDD3] card-hover">
          <AlertTriangle size={28} className="text-[#DC2626] mb-3" />
          <p className="text-3xl font-extrabold text-[#2B1E16]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{stuckSagas}</p>
          <p className="text-sm text-[#9E8E82] font-semibold mt-1">Active Sagas</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-[#E8DDD3]">
        <div className="flex items-center border-b border-[#E8DDD3]">
          <button onClick={() => setTab('users')} className={`px-8 py-4 text-sm font-bold transition-all ${tab === 'users' ? 'text-[#7A2E1A] border-b-2 border-[#7A2E1A]' : 'text-[#9E8E82] hover:text-[#8B7D72]'}`}>
            <Users size={16} className="inline mr-2" />Users
          </button>
          <button onClick={() => setTab('sagas')} className={`px-8 py-4 text-sm font-bold transition-all ${tab === 'sagas' ? 'text-[#7A2E1A] border-b-2 border-[#7A2E1A]' : 'text-[#9E8E82] hover:text-[#8B7D72]'}`}>
            <Heart size={16} className="inline mr-2" />Sagas
          </button>
          <div className="flex-1" />
          {tab === 'users' && (
            <button onClick={() => setShowCreate(true)}
              className="mr-2 flex items-center gap-2 px-4 py-2 bg-[#2D6A4F] text-white rounded-xl text-xs font-bold hover:bg-[#1B4332] transition-all">
              <Plus size={14} /> Add Doctor
            </button>
          )}
          <button onClick={fetchData} disabled={loading}
            className="mr-4 flex items-center gap-2 px-4 py-2 bg-[#FDF6F0] rounded-xl text-xs font-bold text-[#8B7D72] hover:bg-[#E8DDD3] transition-all">
        ...
        {tab === 'users' && (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-[#FDF6F0] hover:bg-[#FAF5EF]">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-[#7A2E1A]/10 rounded-lg flex items-center justify-center text-[#7A2E1A] text-xs font-extrabold">{u.name?.charAt(0)}</div>
                        <span className="font-bold text-sm text-[#2B1E16]">{u.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-[#8B7D72]">{u.email}</td>
        ...
        {tab === 'sagas' && (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {sagas.map((s) => (
                  <tr key={s.id} className="border-b border-[#FDF6F0] hover:bg-[#FAF5EF]">
                    <td className="px-6 py-4 text-sm font-bold text-[#2B1E16]">#{s.id}</td>
                    <td className="px-6 py-4 text-sm text-[#8B7D72]">{s.sagaType}</td>
                    <td className="px-6 py-4 text-center"><span className="status-badge pending">{s.status}</span></td>
                    <td className="px-6 py-4 text-center text-sm text-[#8B7D72]">{s.currentStep || '-'}</td>
                    <td className="px-6 py-4 text-right">
                      {s.status === 'PENDING' && (
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleSagaAction(s.id, 'retry')} className="px-3 py-1.5 bg-[#2D6A4F]/10 text-[#2D6A4F] rounded-lg text-xs font-bold hover:bg-[#2D6A4F]/20 border border-[#2D6A4F]/20">Retry</button>
                ...
                {sagas.length === 0 && <tr><td colSpan="5" className="py-16 text-center text-[#9E8E82] font-medium">No sagas</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-[#E8DDD3]">
              <h3 className="text-lg font-extrabold text-[#2B1E16]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Create Doctor</h3>
              <button onClick={() => setShowCreate(false)} className="text-[#9E8E82] hover:text-[#8B7D72] p-1"><X size={20} /></button>
            </div>
            <form onSubmit={createDoctor} className="p-6 space-y-4">
              <div>
                <label className="text-[10px] font-bold text-[#8B7D72] uppercase tracking-wider mb-1.5 block">Full name</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-[#8B7D72] uppercase tracking-wider mb-1.5 block">Email</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-[#8B7D72] uppercase tracking-wider mb-1.5 block">Password</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowCreate(false)} className="flex-1 py-3 bg-[#FDF6F0] rounded-xl text-sm font-bold text-[#8B7D72] hover:bg-[#E8DDD3] transition-all">Cancel</button>
